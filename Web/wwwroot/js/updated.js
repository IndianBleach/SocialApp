$(document).ready(() => {    
    ...
    const switchToFriendsWindow = (userGuid) => {
        ...
        if (sessionStorage.getItem("friendLoaded") != "true") {
            $("#friendsWindowLoadPrev").removeClass("d-none");
            $.get("/asyncload/user/getfriends", { userGuid }, resp => {
                if (resp != null)
                    sessionStorage.setItem("friendLoaded", true);
                if (resp.length == 0) {
    ...
    $(".showParticipationWindowBtn").on("click", () => {
        ...
        if (sessionStorage.getItem("participationLoaded") != "true") {
            $("#participationWindowLoadPrev").removeClass("d-none");
            $.get("/user/get/participation", {}, resp => {
                sessionStorage.setItem("participationLoaded", true);
                if (resp != null) {
                    $("#participationWindowLoadPrev").addClass("d-none");
                    if (resp.length == 0) {
